...
public class AccountService {
	...
	public Transfer sendTransfer(AuthenticatedUser user,Transfer transfer) {
		HttpHeaders headers = new HttpHeaders();
		headers.setBearerAuth(user.getToken());
		HttpEntity<?> entity = new HttpEntity<Transfer>(transfer, headers);
		ResponseEntity<Transfer> response = restTemplate.exchange(BASE_SERVICE_URL + "transfers/send/", HttpMethod.POST, entity, Transfer.class);
		return response.getBody();
	}
